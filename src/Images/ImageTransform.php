...
class ImageTransform {
   ...
   */
  static function thumbsJpeg($img_dir, $img_name, $thumb_anchura, $thumb_altura='', $thumb_prefijo='') {
    $trazas_obj = 'ImageTransform.inc';

    if(!$img_name) return; // no hace nada

    if(!$thumb_prefijo && $thumb_prefijo != 'NADA') $thumb_prefijo = 'th_';
    else $thumb_prefijo = '';
   ...
   /** Thumbnail **/
    ...
    // Guardar
    if(file_exists($thumb_ruta)) unlink($thumb_ruta);
   ...
   */
  static function crop($img_dir, $img_name, $width, $height, $outputBuffer=true, $cropX='right', $cropY='bottom') {
   /** Datos **/
  ...
  //---------------------------------------------------------------------
  static function getDatosImg($img_dir, $img_name) {
    $ruta = $img_dir.'/'.$img_name;
